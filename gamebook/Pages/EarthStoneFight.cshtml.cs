...
{
    public class EarthStoneFightModel : PageModel
    {
        ...
        public EarthStoneFightModel(ISessionStorage<GameState> ss)
        {
            _ss = ss;
        ...
        public ActionResult OnPost(int type)
        {
            ...
            if (State.CurrentEnemyHP <= 0) // Hráè vyhrál -> redirect na returnUrl
            {
                ...
                //State.Coins += 5;
                State.EarthStoneCount += 1;
                State.EarthStone = true;
                _ss.Save(KEY, State);
                return RedirectToPage("Place", new { id = "Tree" });
            }
